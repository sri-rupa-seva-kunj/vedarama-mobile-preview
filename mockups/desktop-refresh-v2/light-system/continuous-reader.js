...
(() => {
 ...
 function fragment(n){const t=texts[n];return `<section class="continuous-verse" id="reader-verse-${n}" data-reader-verse="${n}"><div class="continuous-verse-heading"><h2>Текст ${n}</h2><button data-linked-verse="${n}" title="Этот текст в других книгах">${icon('book')}<span>В других книгах</span><small>2</small></button></div><section class="text-block" data-block="translation"><h3>Перевод · ${t[0]}</h3><p class="translation">${t[1]}</p></section><section class="text-block" data-block="comment"><h3>Комментарий</h3><p>${t[2]}</p></section></section>`;}
