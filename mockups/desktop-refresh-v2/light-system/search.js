...
const searchModes={exact:'Точный',ai:'AI',both:'Точный + AI'};
...
if(!searchPrefs.exact&&!searchPrefs.ai)searchPrefs.exact=true;
...
$('.global-search input').placeholder='Точный и AI-поиск по библиотеке';
...
function renderSearch(){
  ...
  document.querySelectorAll('[data-search-mode]').forEach(b=>b.onclick=()=>{captureSearchInput();searchMode=b.dataset.searchMode;dirtySearch();renderSearch();});
...
function openSearchSettings(){
  captureSearchInput();
  simpleDialog('Настройки поиска',`<form id="search-preferences-form"><fieldset class="search-pref-modes"><legend>Доступные способы поиска</legend><label><input id="allow-exact" type="checkbox" ${searchPrefs.exact?'checked':''}><span><b>Точный поиск</b><small>Буквальные совпадения в текстах</small></span></label><label><input id="allow-ai" type="checkbox" ${searchPrefs.ai?'checked':''}><span><b>AI-поиск</b><small>Обобщение и источники по вопросу</small></span></label><p>Включите оба способа, чтобы использовать их одновременно.</p></fieldset><label>Режим по умолчанию<select id="default-search-mode"></select></label><fieldset class="search-pref-blocks"><legend>Искать в разделах текста</legend>${Object.entries(searchBlockNames).map(([id,name])=>`<label><span>${name}</span><input type="checkbox" data-search-block="${id}" ${searchPrefs.blocks.includes(id)?'checked':''}></label>`).join('')}</fieldset><p id="search-pref-error" role="alert" hidden></p><div class="editor-actions"><button type="button" id="cancel-search-prefs">Отмена</button><button class="gold-button" type="submit">Применить</button></div></form>`);
  function defaults(){const exact=$('#allow-exact').checked,ai=$('#allow-ai').checked,old=$('#default-search-mode').value||searchPrefs.defaultMode;$('#default-search-mode').innerHTML=Object.entries(searchModes).filter(([id])=>id==='both'?exact&&ai:id==='exact'?exact:ai).map(([id,name])=>`<option value="${id}" ${old===id?'selected':''}>${name}</option>`).join('');}
  $('#allow-exact').onchange=defaults;$('#allow-ai').onchange=defaults;defaults();$('#cancel-search-prefs').onclick=()=>$('#dialog').close();
  $('#search-preferences-form').onsubmit=e=>{e.preventDefault();const exact=$('#allow-exact').checked,ai=$('#allow-ai').checked,blocks=[...document.querySelectorAll('[data-search-block]:checked')].map(x=>x.dataset.searchBlock);if((!exact&&!ai)||!blocks.length){$('#search-pref-error').hidden=false;$('#search-pref-error').textContent='Выберите хотя бы один способ поиска и один раздел текста.';return;}searchPrefs={...searchPrefs,exact,ai,blocks,defaultMode:$('#default-search-mode').value};saved.searchPreferences={...searchPrefs};persist();searchMode=safeSearchMode(searchMode);dirtySearch();$('#dialog').close();renderSearch();};
}
