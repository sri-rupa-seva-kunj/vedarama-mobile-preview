...
function renderReadingBookmarks(){
 const root=$('.reading-bookmarks-body');
 root.innerHTML=`<p class="reading-bookmark-context">${icon('book')}Бхагавад-гита · 2.47</p><button id="reading-add-bookmark" class="outline-button" ${saved.bookmarks.some(b=>b.id==='bg247')?'disabled':''}>${icon('plus')}${saved.bookmarks.some(b=>b.id==='bg247')?'Это место сохранено':'Сохранить это место'}</button><div class="reading-bookmark-list">${saved.bookmarks.length?saved.bookmarks.map(b=>`<article><button data-reading-bookmark="${escapeText(b.id)}"><span>${escapeText(b.book)}</span><strong>${escapeText(b.title)}</strong><p>${escapeText(b.label||'Сохранённое место')}</p></button><button data-reading-bookmark-remove="${escapeText(b.id)}" aria-label="Удалить закладку ${escapeText(b.title)}" title="Удалить закладку">${icon('close')}</button></article>`).join(''):'<div class="empty-state"><h3>Здесь будут важные места</h3><p>Сохраните текущий стих, чтобы быстро вернуться к нему.</p></div>'}</div><button id="reading-all-bookmarks" class="text-button">Все закладки и полки${icon('forward')}</button>`;
 drawIcons(root);
 $('#reading-add-bookmark').onclick=()=>{addBookmark();renderReadingBookmarks();};
 root.querySelectorAll('[data-reading-bookmark]').forEach(b=>b.onclick=()=>{$('.book-scroll').scrollTop=0;if(innerWidth<=700)document.body.classList.remove('reading-bookmarks-open');});
